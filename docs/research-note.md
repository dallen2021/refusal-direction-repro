...
1. **Quality of compliance is not measured.** Our refusal classifier checks whether the first 16 generated tokens begin with a refusal phrase. A response like *"Sure, I can help. SQL injection is..."* counts as non-refusal even if the model then produces vague or evasive content. A stricter judge (HarmBench-style LLM scorer, StrongREJECT) would tighten the behavioral claims and is a natural next step.
